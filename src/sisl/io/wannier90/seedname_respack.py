...
class geomSileWannier90(SileWannier90):
    """Wannier90 centres file ``_centres.xyz``"""
    ...
    def read_geom(self):
        ...
        while True:
            l = self.readline()
            if count < 3:
                cell[count, :] = np.array(list(map(float, l[count].split())))
            else:
                pos = tuple(map(float, l[count].split()))
                orb_pos.append(pos)
            count += 1
            if l == "":
                break
        ...
        return lattice, Geometry(orb_pos, lattice=lattice)
...
add_sile("_geom.dat", geomSileWannier90, gzip=True)
